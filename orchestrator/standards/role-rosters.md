...
- **`unity-developer`** — the single canonical name for this role, full stop. Every generated
  Unity subagent in this scope uses this exact name; do not mint `unity-senior-dev`,
  `senior-unity-dev`, or any other seniority-qualified variant — `coding-standards.md` already
  requires senior-engineer judgment from every role, so a "senior" qualifier on the name is a
  distinction without a difference and only fragments `learnings/roles/` across two files that
  should be one. If a project's repo already has an agent file under a different name from before
  this rule existed, rename it to `unity-developer` (folding its history into
  `learnings/roles/unity-developer.md`) rather than perpetuating the split. Owns gameplay, engine,
  DI wiring, views/UI, presenters, performance — the default owner of anything not carved out below.
